...
object run extends App {
  ...
  if (false) {
    ...
    val pakts = fileOper.toArrays(gPms.op+gPms.pf).filter(i => i(0).contains("Akt")).toArray.slice(1,3)
    ...
    srt ! snpCalcOrderActor.yArray(pakts)
...
//  if (false) {
    ...
    if (false) {
      ...
      //write pval of single snp to pakt473 for the vegas2
      scala.io.Source.fromFile(gPms.op + "snp6Annot.csv").getLines.dropWhile(_.startsWith("#")).map(_.split("\" \"").map(_.replace("\"", "")).slice(0, 2)).map(i => (i(1), pval.getOrElse(i(0), -1f))).filter(_._2 != -1f).filter(_._1.startsWith("rs")).map(i => i._1 + "\t" + i._2).foreach(lpvwriter.println)
      lpvwriter.close()
    }
...
//  }
  // 2018-5-14 MGMT status and/or gene expression as phenotype data
    if (false) {
      ...
      val ch = Array(1 to 22: _*).map(_.toString)
      val orderpms = snpCalcOrderActor.orderPms(k = 3, nactor = 7, pfile = gPms.op + pfl)
      //,efile = "")
      val srt = system.actorOf(snpCalcOrderActor.props(orderpms), "srt")
  ...
  // 2018-5-20 gsea analysis for the results
    if (false) {
      ...
      val orderpms = gseaDispatchActor.pathwayPms(rsFile = rsf, rsord = rsorder)
      //,efile = "")
      val srt = system.actorOf(gseaDispatchActor.props(orderpms), "srt")
      srt ! action
    }
  ...
  // 2019-3-29 comparing VEGAS2  and new GANOVA using simulating SNP within one gene in chr15
  // results are before "simuRs_2018_04_02_07_01_24_582.txt"
    if (false) {
      ...
      val orderpms = simumasterActor.Pms(gPms.rp + "simuRs.txt", 100, Array(0.03f, 0.04f, 0.05f))
      ...
      srt ! simucalculateActor.gList(glist, 4)
