...
//    val glist = rs._3
  ...
  def vegasP(glist:Array[String],Y:DenseMatrix[Float],ognames:String = utils.getTimeForFile+"_"+scala.util.Random.nextInt(100)) = {
    ...
    val ov = Process(vegas2,new File(gPms.tp)).! ProcessLogger(comm6 => stdout = line.filter(i => i.contains(rsn)).apply(0).split(" "))//lineStream.filter(i => i.contains(rsn)).apply(0).split(" ")
